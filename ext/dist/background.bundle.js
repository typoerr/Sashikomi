	...
	*/
	...
	var putMemo = function putMemo(obj) {
		return db.transaction('rw', db.memos, function () {
			return db.memos.put(obj).then(function (id) {
				return db.memos.get(id);
			});
		});
	};

		var deleteMemo = function deleteMemo(id) {
			return db.transaction('rw', db.memos, function () {
				return db.memos.delete(id);
			});
		};

		var getMemosByUrl = function getMemosByUrl(url) {
			return db.transaction('rw', db.memos, function () {
				return db.memos.where('url').equals(url).toArray();
	  });
	};

		deleteMemo(5).then(db.memos.count(function (count) {
